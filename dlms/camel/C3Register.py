...
class C3Register(DlmsClass):
    ...
    def set_value(self, data, isDownloadMode=True):
        ...
        :param data:                    期望值 (数值）
        :param isDownloadMode:          期望值 (数值）
        :return:                        返回一个KFResult对象
        """
        if isDownloadMode:
            if self.obis == '1-0:11.35.0.255':
                data = {"currentThreshold": data}
                return setMPCValue(self.conn, mpcMap=data)
            if self.obis == '1-0:11.44.0.255':
                data = {"currentDuration": data}
                return setMPCValue(self.conn, mpcMap=data)
            if self.obis == '1-0:12.31.0.255':
                data = {"voltageThreshold": data}
                return setMPCValue(self.conn, mpcMap=data)
            if self.obis == '1-0:12.43.0.255':
                data = {"voltageDuration": data}
                return setMPCValue(self.conn, mpcMap=data)
            if self.obis == '1-0:0.8.4.255':
                data = {"loadProfileInterval": data}
                return setMPCValue(self.conn, mpcMap=data)
