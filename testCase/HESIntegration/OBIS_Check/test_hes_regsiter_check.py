...
class Test_HES_Register_Check:
    ...
    def get_db_register(action):
        ...
        items_sql1 = "select PTL_CODE, ITEM_ID, OBIS, CLASS_ID, INDEX_ID, INDEX_TYPE from "
        items_sql2 = " where PTL_CODE = (select ptl_code from (select MODEL_CODE, PTL_CODE, PARENT_PTL_CODE from HES_PARSE_PTL_MODEL) ptl where ptl.model_code = (select METER_MODEL from AM_DEVICE d where d.DEVICE_ADDRESS = '{}'))or PTL_CODE = (select PARENT_PTL_CODE from (select MODEL_CODE, PARENT_PTL_CODE from HES_PARSE_PTL_MODEL) ptl where ptl.model_code = (select METER_MODEL from AM_DEVICE d where d.DEVICE_ADDRESS = '{}'))".format(
