...
class DB:
    ...
    def initial_result(self, meter_no):
        ...
        try:
            ...
            cur = con.cursor()
            cur.execute(
                f"create table {table_name} as select * from h_ptl_register where ptl_type =(select PTL_TYPE from c_ar_model where MODEL_CODE = (select model_code from c_ar_meter where meter_no='{meter_no}'))")
