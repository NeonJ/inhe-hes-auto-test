...
def readFrameCounter(project, clientId):
    """
    从 conf/FrameCounter 目录下的配置文件中读取frameCounter

    :param project:         项目名
    :param clientId:        客户端ID
    :return:                frameCounter
    """
    filename = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "conf/FrameCounter/%s.ini" % project.lower())
...
def writeFrameCounter(project, clientId, frameCounter):
    """
    将 frameCounter 写入到 conf/FrameCounter 目录下的配置文件中
